...
<p align="center">
  ...
  <a href="CHANGELOG.md"><img src="https://img.shields.io/badge/API%20version-1.1.0-informational.svg" alt="API version 1.1.0"></a>
...
Both files describe API version **1.1.0**: 634 paths and 782 operations. The base URL is `https://packetexchange.io/api/v1`.
...
- **Voice and SMS:** send an SMS, place a call, read a one-time passcode by voice call, and the Verify flow (`/verify/start`, `/verify/check`).
...
- API keys look like `wmmn_live_sk_...` (live) and `wmmn_test_sk_...` (test). Some account-management operations accept a dashboard session only and refuse API keys; each operation states its access rule and, for scoped keys, the scope it needs.
...
- Voice passcodes can be spoken in English, Spanish, French, German, Portuguese and Hindi (`en`, `es`, `fr`, `de`, `pt`, `hi`).
